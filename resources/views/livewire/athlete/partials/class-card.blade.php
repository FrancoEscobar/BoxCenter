{{-- Card historial --}}
@if($clase->es_historial ?? false)
@php
    $estado = $clase->asistencia_estado ?? null;
    $badgeClass = $estado === 'asistio' ? 'bg-success' : ($estado === 'ausente' ? 'bg-danger' : 'bg-primary');
    $badgeText = $estado === 'asistio' ? 'Asististe' : ($estado === 'ausente' ? 'No asististe' : 'Reservada');
@endphp
<div wire:click="abrirModal({{ $clase->id }}, true)"
    ...
    <div class="d-flex justify-content-between align-items-start">
        <div>
            ...
            <p class="text-muted small mb-1">Fecha: {{ $clase->fecha }}</p>
